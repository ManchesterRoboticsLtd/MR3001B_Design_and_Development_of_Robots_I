...
class SimpleSystem:
  ...
  def __init__(self):
    ...
    self.max_speed = rospy.get_param("~system_max_output",13.0)
    ...
    self.init_conditions = rospy.get_param("~system_initial_conditions",0) 
    ...
    self.Input.stamp = 0.0


    # Declare the  process output message
    self.output = system_output()
    self.output.output= self.init_conditions
    self.output.stamp = 0.0
    self.SystemStatus(self.init_conditions)
  

    # Setup the Subscribers
    rospy.Subscriber("system_input",system_input,self.input_callback)

    #Setup de publishers
    self.state_pub = rospy.Publisher("system_output", system_output, queue_size=1)
  ...
  #System
      ...
      if dt >= self.sample_time:
        ...
        self.output.stamp = rospy.Time.now()
        self.SystemStatus(self.proc_output)
        #Publish message
  ...
  def SystemStatus(self,speed):
    if (abs(speed)<=abs(self.param_K*self.Input.input*0.8) and abs(self.Input.input)<=self.min_input):
      self.output.status = "System Stoppped"
    elif (abs(speed)>=self.max_speed):
      self.output.status = "System Max Output"
    else:
      self.output.status = "System Normal"
 ...
 #Stop Condition
  ...
  #Setup the stop message (can be the same as the control message)
    ...
    self.output.stamp = rospy.Time.now()
    self.output.status = "System Stopped"
    self.state_pub.publish(self.output)
